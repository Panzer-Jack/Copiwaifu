...
const LANGUAGE_COPY: Record<AppLanguage, LanguageCopy> = {
  [APP_LANGUAGE.ENGLISH]: {
    ...
    settings: {
      ...
      aiTalkLabel: 'AI Talk',
      aiTalkHint: 'After a session completes or errors, generate one short pet bubble from the saved session summary.',
  ...
  [APP_LANGUAGE.CHINESE]: {
    ...
    settings: {
      ...
      aiTalkLabel: 'AI Talk',
      aiTalkHint: '会话完成或出错后，基于已保存的 session 摘要生成一句桌宠气泡短反馈。',
  ...
  [APP_LANGUAGE.JAPANESE]: {
    ...
    settings: {
      ...
      aiTalkLabel: 'AI Talk',
      aiTalkHint: 'セッションの完了またはエラー後、保存済みの要約から短い吹き出しを生成します。',
